 ...
 */
@Component
public class WeakEffectParser extends AbstractSkillEffectParser {
	
	
	protected int getType() {
		return SkillEffectType.WEAKNESS_EFFECT.getCode();
	}
	...
	public void parser(Context context, FightAttribute attacker, 
		...
		int rateBase = AttributeKeys.RATE_BASE;
		int skillDamage = skillEffect.calcSkillEffect(rateBase, skillLevel).intValue();
		if(skillDamage <= 0) {
			if(LOGGER.isDebugEnabled()) {
		...
		context.addAttributeChanges(targetId, AttributeKeys.MOVE_SPEED, 0);
		context.addAttributeChanges(targetId, AttributeKeys.THEURGY_DEFENSE, 0);
		context.addAttributeChanges(targetId, AttributeKeys.PHYSICAL_DEFENSE, 0);
