 ...
 */
@Component
public class DOTPercentAndFixedDamageEffectParser extends AbstractSkillEffectParser {
	
	
	protected int getType() {
		return SkillEffectType.DOT_PERCENT_AND_FIXED_EFFECT.getCode();
	}
	...
	public void parser(Context context, FightAttribute attacker, FightAttribute targeter, SkillEffectConfig skillEffect) {
		...
		int skillDamage = skillEffect.calcSkillEffect(damageValue, skillLevel, damageValue).intValue();
		if(skillDamage <= 0) {
			if(LOGGER.isDebugEnabled()) {
		...
		targeter.addBuffer(Buffer.valueOf(skillEffectId, skillLevel, cycle, skillDamage, cycle, endTime, attackId));
		context.addChangeBuffers(targetId, ChangeBuffer.addDeBuffer(skillEffectId, skillLevel, cycle, cycle, endTime, skillDamage, attackId));
	}
}
