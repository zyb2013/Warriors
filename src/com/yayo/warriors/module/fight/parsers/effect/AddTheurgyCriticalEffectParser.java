 ...
 */
@Component
public class AddTheurgyCriticalEffectParser extends AbstractSkillEffectParser {
	
	
	protected int getType() {
		return SkillEffectType.ADD_THEURGY_CRITICAL.getCode();
	}
	...
	public void parser(Context context, FightAttribute attacker, 
		...
		if(unitBuffer != null) {
			int removeBufferId = unitBuffer.getId();
			targeter.removeBuffer(removeBufferId);
			context.addFightReport(FightReport.buffer(targetId, skillEffectId, 0));
			context.addChangeBuffers(targetId, ChangeBuffer.removeBuffer(removeBufferId));
		}
