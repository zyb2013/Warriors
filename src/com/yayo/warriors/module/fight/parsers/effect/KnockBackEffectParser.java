 ...
 */
@Component
public class KnockBackEffectParser extends AbstractSkillEffectParser {
	
	
	protected int getType() {
		return SkillEffectType.KNOCK_BACK_EFFECT.getCode();
	}
	...
	public void parser(Context context, FightAttribute attacker, FightAttribute targeter, SkillEffectConfig skillEffect) {
		...
		if(targeter.isDead()) {
			if(LOGGER.isDebugEnabled()){
				LOGGER.debug("被攻击者: [{}] 已经死亡, 不能接受击退效果.");
			}
			return;
		}
		...
		if(BufferHelper.isUnitInImmobilize(attacker.getBuffers())) { 
			if(LOGGER.isDebugEnabled()){
				LOGGER.debug("攻击者: [{}] 有定身效果, 不能发起击退效果.");
			}
			return;
		} 
		...
		if(currentValue <= 0) {
			if(LOGGER.isDebugEnabled()){
				LOGGER.debug("攻击方:[{}] 发起攻击. 被攻击方:[{}] 已经死亡, 不能发起击退效果", attackId, targetId);
			}
			return;
		}
		
		if(targeter.isKnockBackDefense()) {
...
//		}
		...
		GameMap gameMap = context.getGameMap();												//地图上下文对象
		Point casterPoint = context.getCasterPoint();										//施法者的位置
		Point targetPoint = context.getTargetPoint();										//目标者的位置
		boolean knockFlyDefense = targeter.isKnockFlyDefense();								//是否可以被击飞
		Point newPoint = this.calcTargetKnockPoint(casterPoint, targetPoint, gameMap);		//新的位置
		...
		context.addUnitHurtValue(targeter.getiSpire(),finalAttributeValue);					//更新最后伤害值
