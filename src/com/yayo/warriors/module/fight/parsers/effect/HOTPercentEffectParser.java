 ...
 */
@Component
public class HOTPercentEffectParser extends AbstractSkillEffectParser {
	
	
	protected int getType() {
		return SkillEffectType.HOT_PERCENT_EFFECT.getCode();
	}
	...
	public void parser(Context context, FightAttribute attacker, 
		...
		AttackDamageVO damageVO = AttackDamageVO.valueOf(isCritical, attacker, targeter);
		int damageValue = FightRule.calculateFightingAttackDamage(damageVO);
		int skillEffectValue = skillEffect.calcSkillEffect(damageValue, skillLevel).intValue();
		double restrainTreatRates = FightHelper.getRestrainTreatRates(attackerBuffers.values());
		int hotAddHpValue = (int)(Math.max(FightRule.MIN_FIGHT_DAMAGE, skillEffectValue - skillEffectValue * restrainTreatRates));
		if(hotAddHpValue < 0) {
		...
		context.addFightReport(FightReport.attack(targetId, skillEffectId, attribute, hotAddHpValue, isCritical, false));
	}
}
