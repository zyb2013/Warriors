 ...
 */
@Component
public class RestrainTreatmentEffectParser extends AbstractSkillEffectParser {
	
	
	protected int getType() {
		return SkillEffectType.RESTRAIN_HOT_EFFECT.getCode();
	}
	...
	public void parser(Context context, FightAttribute attacker, 
		...
		int baseRate = AttributeKeys.RATE_BASE;
		int skillDamage = skillEffect.calcSkillEffect(baseRate, skillLevel).intValue();
		if(skillDamage <= 0) {
			if(LOGGER.isDebugEnabled()) {
		...
		context.addChangeBuffers(targetId, ChangeBuffer.addDeBuffer(skillEffectId, skillLevel, cycle, endTime, skillDamage, attackId));
	}
