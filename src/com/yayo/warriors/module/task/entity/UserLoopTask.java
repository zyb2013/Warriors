...
@Entity
@Table(name="userLoopTask")
public class UserLoopTask extends BaseModel<Long> {
	 ...
	 */
	public static UserLoopTask valueOf(long playerId) {
		UserLoopTask userTask = new UserLoopTask();
	...
	public void refreshTask(TaskCondition loopTaskInfo, int quality, int playerLevel) {
		...
		this.taskLevel = Math.max(TaskRule.MIN_LOOP_TASK_LEVEL, playerLevel);
	...
	public int getTaskLevel() {
		return Math.max(TaskRule.MIN_LOOP_TASK_LEVEL, taskLevel);
	}
	 ...
	 * @return
	 */
	public String buildRewardInfoString(Collection<LoopRewardConfig> loopRewards) {
		StringBuilder builder = new StringBuilder() ;
		if(loopRewards == null || loopRewards.isEmpty()) {
			return builder.toString();
		}
		
		Set<Integer> rewardInfos = this.getRewardInfoSet();
		for(LoopRewardConfig loopRewardConfig : loopRewards) {
			if(!loopRewardConfig.canReward()) {
				continue;
			}
			
			int rewardId = loopRewardConfig.getId();
			int state = rewardInfos.contains(rewardId) ? 0 : 1;
			for (RewardVO rewardVO : loopRewardConfig.getRewardList()) {
	...
	public boolean isAccepted() {
		return this.status == TaskStatus.ACCEPTED || this.status == TaskStatus.COMPLETED;
	}
